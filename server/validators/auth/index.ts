...
export const SignupQuerySchema = {
    body: Joi.object({
        firstname: Joi.string().regex(/^[a-zA-ZÀ-ÖØ-öø-ÿ]{2,24}$/).required(),
        lastname: Joi.string().regex(/^[a-zA-ZÀ-ÖØ-öø-ÿ]{2,24}$/).required(),
...
export const TokenVerificationRequestSchema = {
    ...
    cookies: Joi.object({
        session: Joi.string().required().custom((value, helpers) => {
            try {
