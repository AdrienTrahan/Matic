...
export class AuthService {
    ...
    async signup(firstname: string, lastname: string, email: string, password: string) {
        ...
        let signupRequest = await this.databaseService
            .call("auth", "signup_user", [firstname, lastname, email, hashedPassword])
            .catch(err => {
    ...
    async recover(email: string) {
        ...
        let recoverRequest = await this.databaseService
            .call("auth", "recover_password", [email, recoveryCode])
            .catch(err => {
                throw UNEXPECTED_ERROR_OCCURED
            })
    ...
    async resetPassword(email: string, password: string, recovery: string) {
        ...
        let hashedPassword = bcrypt.hashSync(password, salt)
        let resetRequest = await this.databaseService
            .call("auth", "reset_password", [email, hashedPassword, recovery])
            .catch(err => {
                throw UNEXPECTED_ERROR_OCCURED
            })
    ...
    async refresh(token: string, userId: string, refresh: string) {
        let refreshValidRequest = await this.databaseService
            .call("auth", "validate_refresh", [token, userId, refresh])
            .catch(error => {
                throw UNEXPECTED_ERROR_OCCURED
            })
        if (
            refreshValidRequest.length == undefined ||
            refreshValidRequest.length == 0 ||
            refreshValidRequest[0].count == undefined
        ) {
            throw UNEXPECTED_ERROR_OCCURED
    ...
    async generateTokenForUser(userId: string) {
        ...
        await this.databaseService.call("auth", "create_token", [token, userId, refresh_token]).catch(err => {
            throw UNEXPECTED_ERROR_OCCURED
        })
        let cookie = {
            token: token,
