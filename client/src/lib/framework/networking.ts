...
export async function safeFetch(...args: any[]){
    ...
    return await (fetch as any)(...args).then((data) => {
        if (data.ok){
            return data.json().then((data) => [data, null]).catch((error) => [null, error])
        }else{
            return data.json().then((data) => [null, data]).catch((error) => [null, error])
        }
    }).catch((error) => {
        return [null, error]
    });
