...
export class EmailService{
    ...
    private init(){
        this.transporter = this.hasRequireEnvironnementVariables() ? nodemailer.createTransport({
            service: 'gmail',
            auth: {
    ...
    private getFile(html: boolean, messageType: string){
        if (html){
            return fs.readFileSync(`./server/services/email-service/templates/compiled/${emailTemplates[messageType].path}.html`, "utf-8");
        }else{
            return fs.readFileSync(`./server/services/email-service/templates/plaintext/${emailTemplates[messageType].path}.txt`, "utf-8");
        }
    }
